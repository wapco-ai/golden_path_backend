...
class PublicJwtService
{
    ...
    public function __construct()
    {
        ...
        $this->issuer    = env('JWT_PUBLIC_ISSUER', 'goldenpath-public-api');
        $this->audience  = env('JWT_PUBLIC_AUDIENCE', 'goldenpath-public-app');
        $this->accessTtl = (int) env('JWT_PUBLIC_ACCESS_TTL', 1800);
    }
    ...
    public function decode(string $token): ?array
    {
        ...
        $now = time();
        if (($payload['iss'] ?? null) !== $this->issuer) return null;
        if (($payload['aud'] ?? null) !== $this->audience) return null;
    ...
    protected function encode(array $payload): string
    {
        ...
        return $data . '.' . $this->base64UrlEncode($sig);
    }
}
